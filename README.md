...
- **부서 격리가 기본값** — 타 부서의 현황·파일·존재까지 서로 보이지 않는다(404).
  "누가 냈는지 감시"가 구조적으로 불가능하다. 격리 테스트가 릴리스 게이트 (AU-14)
- **역할**: member(업로드) / lead(현황·드로어 열람·규칙·병합·양식 관리) / operator(테넌시만, 내용 접근 불가)
- **부서가 자기 것을 관리** — 양식·병합 규칙(텍스트)·마감 정책·제출 대상 명단
...
| 전사 수합 구조 | 2단계 (부서 내부 → 취합게시판, 전사 마감 수 15:00 관찰) |
...
- 🔴 **Q-11** — 테넌트 경계(1부서=1보고 권고안) 확인 → 즉시 Phase 1 착수 가능
- 🔴 **Q-12** — AI홍보전략실 초기 제출 대상(13명 중) 확정
- 🔴 **Q-01** — `fixtures/verify-write-test.hwp` 한글에서 열어보기 *(Phase 2 전까지)*
