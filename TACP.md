...
버전 1.4 · 2026-09-03 · 대상 코드 v1.32.0
...
- v1.4 — **TACP-18 신설**: 마감 예외는 「문을 잠시 여는 것」이다. 담당자가 남을 대신해
...
### TACP-18 — 마감 예외는 **문을 여는 것**이지 대신 내주는 것이 아니다 (v1.4 신설)

늦게 낸 사람을 담당자가 받아 줄 수 있어야 한다. 방법이 둘인데, 하나만 고른다.

| | 기록 | |
|---|---|---|
| ~~담당자가 대신 올리기~~ | 「누구 것인가」와 「누가 올렸나」가 **갈린다** | 채택하지 않음 |
| **마감을 잠시 열기** | 낸 사람이 **자기 이름으로** 낸다 | 채택 |

**왜 갈리면 안 되는가.** 갈린 기록은 화면 어느 한 곳에서 표시를 빠뜨리는 순간 거짓말이 된다.
그 화면은 나중에 얼마든지 늘어나고(현황·이력·병합본 작성자·감사 로그), 한 곳만 빠져도
「그 사람이 낸 것」으로 읽힌다. **되돌릴 수 없는 종류의 거짓**이라 그 갈림 자체를 만들지 않는다.

문을 여는 쪽은 갈릴 것이 없다 — 낸 사람이 낸 것이다.

#### 규칙

- **누가**: `lead`·`head`·`operator`. `coordinator`는 제외한다 — 전사 취합은 읽기이고
  (TACP-8), 남의 부서 마감을 열 이유가 없다. 게이트는 `requireOwnManager` (§3.2와 같다)
- **어느 부서**: 신원의 부서만. 본문·URL이 정하지 않는다 (TACP-6)
- **얼마나**: `OPEN_MINUTES`(30분). **시각이 지나면 저절로 닫힌다** —
  열어 두고 잊는 것이 이 기능의 유일한 위험이고, 사람이 기억해야 지켜지는 규칙은
  언젠가 안 지켜진다. 그래서 「연다」가 아니라 **「언제까지」**로 만들었다
- **보이게**: 열려 있는 동안 담당자 화면에 경고 띠, 부서원 화면에 「마감 후 열림 · HH:mm까지」.
  예외가 조용하면 예외인 줄 모른다 (TACP-9)
- **남게**: `deadline_open` · `deadline_close` 감사 기록. **예외에는 언제나 이름이 붙는다**

#### 마감이 사라지는 것이 아니다

「마감 예외 없음」이 「담당자가 이름을 걸고 연 30분」으로 바뀐 것이다.
부서원이 스스로 여는 길은 없고(§3.1에서 member는 `—`), 열린 사실은 화면과 기록에 남는다.
...
v1.1까지 member는 병합본을 못 받았다. 근거는 "남의 업무 내용이 담겨 있다"였다.
**그 근거가 틀렸다** — 병합본은 취합게시판에 올라가 **전사가 보는 문서**다.
자기가 쓴 글이 들어간 문서를 정작 본인만 못 보고, 다른 부서 사람은 게시판에서 보는
상태였다. 숨겨서 지키는 것이 없었다.

그래서 **내 부서 병합본은 부서원 모두가 열람·내려받는다.** 부서원이 결과물을 보면
다음 주에 무엇을 어떻게 쓸지 감이 잡히고, 잘못 들어간 것도 본인이 먼저 발견한다.

**이 개정이 TACP-11과 충돌하지 않는 이유 — 병합본에는 작성자가 없다.**

병합본은 «누가 썼는가»를 담지 않는다. 표에 남는 것은 업무 내용뿐이고,
합쳐진 문서 어디에도 제출자 이름이 붙지 않는다. 그래서 부서원이 보는 것은
**부서의 결과물**이지 남의 제출물이 아니다 — TACP-11(내용은 lead부터)이 지키려던
«누가 무엇을 썼는지»는 그대로 닫혀 있다.

부서원이 이걸 보는 목적도 거기에 있다: **내가 낸 내용이 제대로 들어갔는지 확인**하는 것.
남이 뭘 냈는지 알 필요가 없고, 알게 되지도 않는다.

작성자가 드러나는 곳은 **담당자 화면의 병합 검토 패널** 하나다 (합쳐진 행에
«누가 + 누가»와 각자의 원문이 나온다). 그건 병합이 옳게 됐는지 판단하는 데 필요한 정보라
`requireLead` 뒤에 있다. 보관함은 그 패널을 쓰지 않는다.

바뀌지 않은 것:
- **수정은 여전히 담당자만** (TACP-6 — 쓰기는 신원의 부서에만, 그리고 lead)
- **개별 제출물은 그대로 lead부터** (TACP-11). 병합본은 부서의 산출물이고
  개별 제출물은 그 사람이 쓴 원본이다 — 둘은 다른 자원이다
- **작성자 정보(병합 검토 패널)는 담당자만** — 보관함에 노출되지 않는다
- **타 부서 병합본은 readAll만** (TACP-7·10)
...
| **DM-T31** | **member가 마감을 열려 하면 404** — 자기 마감을 자기가 여는 건 마감이 아니다 |
| **DM-T32** | 담당자가 연 뒤 제출하면 **제출자는 그 부서원 본인** (TACP-18) |
| **DM-T35** | **남의 부서 마감은 못 연다** — 부서는 신원에서 나온다 (TACP-6) |
| **DM-T36** | 열면 감사 기록에 **누가 열었는지** 남는다 |
