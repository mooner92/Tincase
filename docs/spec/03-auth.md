...
### AU-17 — 대상 부서 해석의 단일 출처 ★ (v1.3.1 신설)

부서 스코프 데이터는 **반드시** `resolveTargetDivision(scope, slugParam?)`을 통해 얻는다.

```
요청 없음 / 내 슬러그 / 내 별칭 → 내 부서 (isOwn=true)
타 부서                        → readAll(operator·coordinator)만 + 감사 로그
그 외                          → 404 (없는 부서와 구별 불가)
```

**왜 규칙으로 못박는가** — v1.3.0에서 실제 사고가 났다. 레이아웃만 URL 슬러그를 해석하고
페이지는 `scope.division`(내 부서)을 써서, **헤더는 국가기후위기적응센터인데 본문 제출 현황은
AI홍보전략실**이 나왔다. 권한 구멍은 아니었지만 (데이터는 본인이 볼 수 있는 것) **오독을 유발하는
치명적 UI 버그**다. 운영자가 남의 부서 현황을 자기 부서로 착각하면 판단을 그르친다.

| 규칙 | 내용 |
|---|---|
| AU-17a | 페이지는 `getDivisionView(slug)`가 준 `division`만 쓴다. `scope.division` 직접 사용 금지 |
| AU-17b | 부서 스코프 API는 `?division=` 를 받되 **readAll일 때만** 타 부서를 허용. 파라미터 없으면 내 부서(안전한 기본값) |
| AU-17c | 타 부서 열람 시 헤더 배지 + 상단 배너로 **항상 명시**한다 |
| AU-17d | 타 부서에서는 제출·양식 내려받기·설정 편집이 **불가능**하다 (읽기 전용) |

AU-17d의 근거: 업로드 부서는 신원에서 도출되므로(DM-12) 타 부서 페이지의 업로드 버튼은
동작해도 **내 부서에 저장된다**. 그런 버튼은 존재 자체가 함정이라 렌더하지 않는다.
설정 편집도 같은 이유로 막는다 — 실수로 내 부서 양식·규칙을 덮어쓸 수 있다.
