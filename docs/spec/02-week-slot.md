...
### WS-16 — 예시도 오늘 날짜에서 만든다

안내 문구에 `2026년 5월은 31일이 일요일이라…`처럼 특정 연도를 적어두면
**해가 바뀌는 순간 낡은 안내**가 된다. 규칙 자체가 날짜에서 계산되므로 예시도 그렇게 만든다.

`monthlyMondayOf(year, month)` — 그 달 월간 주의 월요일. 사용 안내의 FAQ가 이걸로
**이번 달·다음 달**을 그때그때 만든다. 몇 년 뒤에 열어도 그때 기준으로 나온다.

### 장기 불변식

로직에 연도가 박혀 있으면 여기서 깨진다.

| ID | 검증 (2020~2060 · 492개월 전수) |
|---|---|
| WS-T29 | 모든 달에 월간 주가 **정확히 하나** |
| WS-T30 | 월간 주에 그 달 **말일**이 들어 있다 (윤년 2/29 표본 포함) |
| WS-T31 | 주차 번호는 1~5, 한 달의 월요일은 4개 또는 5개, **마지막 월요일 = 월간 주** |
