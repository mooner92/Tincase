...
export function PasswordForm({ first, hasPassword }: { first: boolean; hasPassword: boolean }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      {hasPassword && (
        <div>
          <label htmlFor="cur" className="mb-1 block text-xs font-medium text-slate-600">
      ...
      <div>
        <label htmlFor="new" className="mb-1 block text-xs font-medium text-slate-600">
          새 비밀번호 <span className="font-normal text-slate-400">(10자 이상)</span>
        </label>
        <input
          id="new"
          type="password"
      ...
      <div>
        <label htmlFor="confirm" className="mb-1 block text-xs font-medium text-slate-600">
          새 비밀번호 확인
        </label>
        <input
          id="confirm"
      ...
      <p className="text-center text-xs text-slate-400">변경하면 다른 기기의 로그인은 모두 해제됩니다.</p>
    </form>
  );
}
