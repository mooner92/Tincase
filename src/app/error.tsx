...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center">
      <h1 className="text-lg font-bold text-slate-800">문제가 발생했습니다</h1>
      <p className="mt-2 text-sm text-slate-600">잠시 후 다시 시도해 주세요. 계속되면 운영자에게 알려주세요.</p>
      <button
        onClick={reset}
      ...
      <p className="mt-3 text-xs text-slate-400">(오류 코드: {code})</p>
    </main>
  );
}
