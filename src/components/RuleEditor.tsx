...
export function RuleEditor({
  ...
  return (
    <div className="space-y-4">
      <div>
        <label htmlFor="guide" className="mb-1 block text-xs font-medium text-slate-600">
        ...
        <textarea
          ...
          rows={4}
          className="w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-xs leading-5"
          placeholder={'한 줄에 하나씩 적습니다.\n예) 상시 반복 업무는 일자를 공란으로 둡니다'}
        />
      </div>
      <div>
        <label htmlFor="rule" className="mb-1 block text-xs font-medium text-slate-600">
          병합 규칙{' '}
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="rounded bg-slate-800 px-4 py-1.5 text-xs font-medium text-white hover:bg-slate-700 disabled:opacity-40"
        >
          {saving ? '저장 중…' : '저장'}
        </button>
        <span aria-live="polite" className={`text-xs ${msg?.ok ? 'text-green-700' : 'text-red-700'}`}>
          {msg?.text}
        </span>
      </div>
    </div>
